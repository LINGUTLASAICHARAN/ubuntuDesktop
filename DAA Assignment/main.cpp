...
bool  isInside(Point* p ,vector<Point*> polygon){
    int n = polygon.size();
    if(n == 0) return true; // newly added
    for (int i = 0; i < n-1; i++)
    {
...
void remove_side(Point* p,  Point* p1, vector<Point*>& L){
    if(L.size() == 0) return;
...
// int pos(vector<Point*> px , Point * k){
        ...
        while(poly.size() > 3){
            ...
            if(L[m].size() != poly.size()){
                ...
                // while(notch.size() > 0){
                    ...
                    // cout<<"size"<<s<<endl;
                    for(int i = 0; i < s; i++){
                        Point * tempor = notch[i];
                        bool t = isInside(notch[i], L[m]);
                        if(t){
                            remove_side(notch[i], temp[1], L[m]);
                            if(notch[i] == NULL){
                                
                            }
                            // auto it = find(notch.begin(),notch.end(),tempor);
                            // if(it != notch.end())
                                // notch.erase(it);
                        }
                        // else{
